...
public class ChannelRepository {
	...
	public void deleteChannelDb(String channelName) {
		Query query = em.createQuery("DELETE FROM ChannelModel chm WHERE chm.name=:cn ");
		query.setParameter("cn", channelName);
		query.executeUpdate();
	}

	public void deleteRelationsDb(String channelName) {
		Query queryDeleteRelations = em.createQuery("DELETE FROM UsersChannels uc WHERE uc.channel=:cn ");
	...
	public void removeUserFromRoomDb(UsersChannels relation) {
		Query query = em.createQuery("DELETE FROM UsersChannels ch WHERE ch.username=:cu AND ch.channel = :ch ");
	...
	public ChannelModel findChannelDb(String channelName) {
		Query queryFindChannel = em.createQuery("SELECT ch FROM ChannelModel ch WHERE ch.name=:ch ");
	...
	public UsersChannels findUserInChannelDb(UsersChannels relations) {
		Query query = em.createQuery("SELECT cm FROM UsersChannels cm WHERE cm.username=:cu AND cm.channel=:cn ");
